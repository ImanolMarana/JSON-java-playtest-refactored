...
public class CDL {
    ...
    public static String rowToString(JSONArray ja, char delimiter) {
        ...
        for (int i = 0; i < ja.length(); i += 1) {
          ...
          appendValue(ja, sb, i, delimiter);
        }
        sb.append('\n');
        return sb.toString();
      }
      
      private static void appendValue(JSONArray ja, StringBuilder sb, int i, char delimiter) {
        Object object = ja.opt(i);
        if (object != null) {
          String string = object.toString();
          if (string.length() > 0 && needsQuotes(string, delimiter)) {
            appendQuotedString(sb, string);
          } else {
            sb.append(string);
          }
        }
      ...
      private static boolean needsQuotes(String string, char delimiter) {
        return string.indexOf(delimiter) >= 0 || string.indexOf('\n') >= 0 || string.indexOf('\r') >= 0 ||
            string.indexOf(0) >= 0 || string.charAt(0) == '"';
      }

      private static void appendQuotedString(StringBuilder sb, String string) {
        sb.append('"');
