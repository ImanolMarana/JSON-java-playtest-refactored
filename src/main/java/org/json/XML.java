...
public class XML {
    ...
    private static String toString(final Object object, final String tagName, final XMLParserConfiguration config, int indentFactor, int indent)
        ...
        if (object instanceof JSONObject) {
            ...
            // Emit <tagName>
            emitOpeningTag(sb, tagName, indentFactor, indent);

            // Emit content in body
            emitJsonObjectBody(jo, sb, tagName, config, indentFactor, indent);

            if (tagName != null) {
                // Emit the </tagName> close tag
                emitClosingTag(sb, tagName, indentFactor, indent);
            }
            return sb.toString();
    ...
    private static void emitJsonObjectBody(JSONObject jo, StringBuilder sb, final String tagName,
            final XMLParserConfiguration config, int indentFactor, int indent) throws JSONException {
        for (final String key : jo.keySet()) {
            ...
            if (key.equals(config.getcDataTagName())) {
                // Emit content in body
                emitCDataContent(sb, value);
            } else if (value instanceof JSONArray) {
                // Emit an array of similar keys
                emitJsonArray(sb, key, (JSONArray) value, config, indentFactor, indent);
            } else if ("".equals(value)) {
                // Emit empty tag
                emitEmptyTag(sb, key, config, indentFactor, indent);
            } else {
                // Emit a new tag <k>
    ...
    private static void emitOpeningTag(StringBuilder sb, final String tagName, int indentFactor, int indent) {
        if (tagName != null) {
    ...
    private static void emitClosingTag(StringBuilder sb, final String tagName, int indentFactor, int indent) {
    ...
    private static void emitCDataContent(StringBuilder sb, Object value) throws JSONException {
    ...
    private static void emitJsonArray(StringBuilder sb, final String key, JSONArray ja,
            final XMLParserConfiguration config, int indentFactor, int indent) throws JSONException {
        int jaLength = ja.length();
        // don't use the new iterator API to maintain support for Android
        for (int i = 0; i < jaLength; i++) {
    ...
    private static void emitEmptyTag(StringBuilder sb, final String key, final XMLParserConfiguration config,
