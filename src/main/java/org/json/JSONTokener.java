...
public class JSONTokener {
    ...
    public String nextString(char quote, boolean strictMode) throws JSONException {
        ...
        for (;;) {
            ...
            switch (c) {
                ...
                case '\\':
                    sb.append(handleEscapeCharacter(this));
                    break;
                default:
                    if (isUnbalancedQuote(c, quote, strictMode)) {
                        throw this.syntaxError(getUnbalancedQuoteMessage(quote));
                    }
    ...
    private boolean isUnbalancedQuote(char currentCharacter, char quote, boolean strictMode) {
        return strictMode && (currentCharacter == '\"' || currentCharacter == '\'') && quote != currentCharacter;
    }

    private String getUnbalancedQuoteMessage(char quote) {
        return String.format(
                "Field contains unbalanced quotes. Starts with %s but ends with a different quote.", quote);
    }

    private String handleEscapeCharacter(JSONTokener tokener) throws JSONException {
        char c = tokener.next();
        switch (c) {
            case 'b':
                return "\b";
            ...
            case 'r':
                return "\r";
            case 'u':
                return handleUnicodeEscape(tokener);
            case '"':
            case '\'':
            case '\\':
            case '/':
            ...
            default:
                throw tokener.syntaxError("Illegal escape. Escape sequence  \\" + c + " is not valid.");
        }
    }

    private String handleUnicodeEscape(JSONTokener tokener) throws JSONException {
        String next = tokener.next(4);
        try {
            return String.valueOf((char) Integer.parseInt(next, 16));
